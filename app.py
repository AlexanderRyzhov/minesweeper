from tkinter import Frame, Button, Label, Spinbox, StringVar, RIGHT, BOTTOM, LEFT, TOP, X, SUNKEN  # noqa: E501
...
class MinesweeperCell:
    ...
    def nextMark(self):
        if self.state in self.markSequence:
            ...
            self.state = self.markSequence[(stateIndex + 1) % len(self.markSequence)]  # noqa: E501
...
class MinesweeperModel:
    ...
    def getCell(self, row, column):
        if row < 0 or column < 0 or self.rowCount <= row or self.columnCount <= column:  # noqa: E501
            return None
    ...
    def isWin(self):
        for row in range(self.rowCount):
            for column in range(self.columnCount):
                ...
                if not cell.mined and (cell.state != 'opened' and cell.state != 'flagged'):  # noqa: E501
                    return False
...
class MinesweeperView(Frame):
    def __init__(self, model, controller, parent=None):
        ...
        Button(panel, text='Новая игра', command=self.controller.startNewGame).pack(side=RIGHT)  # noqa: E501
    ...
    def getGameSettings(self):
        return self.rowCount.get(), self.columnCount.get(), self.mineCount.get()  # noqa: E501
        ...
        for row in range(self.model.rowCount):
            ...
            for column in range(self.model.columnCount):
                btn = Button(
                    ...
                    width=2,
                    height=1,
                    command=lambda row=row, column=column: self.controller.onLeftClick(row, column),  # noqa: E501
                    padx=0,
                    pady=0
                )
                btn.pack(side=LEFT)
                btn.bind(
                    '<Button-3>',
                    lambda e, row=row, column=column: self.controller.onRightClick(row, column)  # noqa: E501
                )
...
class MinesweeperController:
    ...
    def startNewGame(self):
        ...
        try:
            ...
            self.model.startGame(self.model.rowCount, self.model.columnCount, self.model.mineCount)  # noqa: E501
    ...
    def onRightClick(self, row, column):
        ...
        self.view.blockCell(row, column, self.model.getCell(row, column).state == 'flagged')  # noqa: E501
        self.view.syncWithModel()
